...
function isFocusRotationInput(keys) {
  return (
    isSelectedTower() &&
    !isShiftDown(keys) &&
    !isAltDown(keys) &&
    (isDown(keys, "q", "Q", "KeyQ") || isDown(keys, "e", "E", "KeyE"))
  );
}
...
export function rotateSelectedObject(keys) {
  ...
  const focusSpeed = state.focusRotationSpeed ?? 0.045;
  const rotateSpeed = 0.035;

  if (!isShiftDown(keys)) {
    if (isDown(keys, "q", "Q", "KeyQ")) {
      rotateTowerFocus(selected, focusSpeed);
    }
