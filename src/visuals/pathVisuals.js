...
export function initPathVisuals(scene) {
  ...
  scene.traverse((object) => {
    ...
    if (object.userData?.isPathBorder) {
      pathBorderMeshes.push(object);
    }
  });
}

export function updatePathVisuals() {
  if (pathMeshes.length === 0 && pathBorderMeshes.length === 0) return;
  ...
  for (const tile of pathMeshes) {
    ...
    if (shouldHighlight && isActive) {
      ...
      setMaterialOpacity(tile.material, tile.userData.pathOpacity ?? 0.42);
      setMaterialEmissive(tile.material, 0x000000);
    }
  }

  for (const border of pathBorderMeshes) {
    const isActive = activePathSet.has(border.userData.pathKey);

    if (shouldHighlight && isActive) {
      setMaterialColor(border.material, 0xfacc15);
      setMaterialOpacity(border.material, 0.95);
      setMaterialEmissive(border.material, 0x78350f);
    } else {
      setMaterialColor(border.material, border.userData.baseColor ?? 0x4b5563);
      setMaterialOpacity(border.material, border.userData.borderOpacity ?? 0.72);
      setMaterialEmissive(border.material, 0x000000);
    }
  }
}
